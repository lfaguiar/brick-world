...
export class ManterBrickComponent implements OnInit {
  ...
  constructor(private tijService: TijoloService, private angularFire: AngularFireDatabase) {
    ...
    this.minhasPecas = angularFire.list('blocos').valueChanges();
  ...
  gravarPecas(gravar) {
    this.angularFire.list("blocos").push(
      {
        nome: 'lucas'
      }
